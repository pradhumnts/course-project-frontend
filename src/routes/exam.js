...
export default function BasicTabs() {
  ...
  const [secondsPerQuestion, setSecondsPerQuestion] = useState(60);
        ...
        subject.map(sub => {
            sub.system.map((sys) => {
                    ...
                    sub.topics.map((topic) => {
                        return topic.map((t, index) => topics.push({ id: t.id, system_id: t.system_id, subject_id: t.subject_id, topic: t.topic, topicAttribute: t.topicAttribute, count: 0, length: sub.questions_length[index], checked: false}) )   
                    })
                    ...
                    systems.push({ id: sys.id, subject_id: sub.id, system: sys.system, active: true, topics: res, count: 0, checked: false})
                })
                subs.push({ id: sub.id, subject: sub.subject, systems: systems, count: sumArr(sub.questions_length), questions_length: sub.questions_length, checked: false})
            })
            ...
            let result = tops.filter((value, index, self) => index === self.findIndex((t) => ( t.id === value.id && t.subject_id === value.subject_id )))
            ...
            result.map(system => {
                if(rs.findIndex(x => x.system === system.system) < 0){
                    ...
                    let systemRsObjectIndex = rs.findIndex(x => x.system === system.system)
                    rs[systemRsObjectIndex].topics = [...rs[systemRsObjectIndex].topics, ...systemResultObject.topics]
                }
         
            })

            rs.map(system => {
                system.topics = system.topics.filter((value, index, self) =>
    ...
    const updateCount = (subject) => {
        ...
        let systems = subject.systems.filter(x => x.subject_id === subject.id)
        
        if(systems.length > 0){
            systems.map(x => x.topics.map(y => {
                if(y.subject_id === subject.id){
                    co.push({"topic": y.topic, "count": y.length, "system": x.system})
                }
            }))
            ...
            systems.map(system => {
                result = sys.map(s => {
                    if(s.system === system.system && s.subject_id === subject.id){
                        s.count += sumArr(co.filter(t => t.system === s.system).map(x => x.count))
                        return s
                    }
                    return s
                })
  ...
  const handleSystemCheckboxChange = (event) => {
    ...
    subjects.map((y) => {
        if (topics.find((z) => z.system === event.target.value)) {
          let tops = topics.find((z) => z.system === event.target.value);
  ...
  let content = {
      ...
      description: "Save Soil is a global movement launched by Sadhguru, to address the soil crisis by bringing together people from around the world to stand up for Soil Health, and supporting leaders of all nations to",
      sections: [
        {
          id: 1,
                    ...
                    <FormControlLabel
                      ...
                      control={
                        ...
                        checked={subject.checked}
                          ...
                          onChange={(event) => handleSubjectCheckboxChange(event, subject)}
                        />
                      }
                    />
                    <Chip
                        ...
                        {system.topics.map((topic, index) => (
                                ...
                                <FormControlLabel
                                    ...
                                    <Checkbox
                                        ...
                                        onChange={(event) => handleTopicCheckboxChange(topic, event)}
                                    />
                                    }
                                />
                                <Chip
